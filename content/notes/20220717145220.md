...
-   [📝bitFlyer bot開発]({{< relref "20220719113407.md#bitflyer-bot開発" >}})
-   [📝FTX bot開発]({{< relref "20220722191127.md#ftx-bot開発" >}})
...
[まちゅけん]({{< relref "20220712161715.md#まちゅけん" >}})さん作成.
...
## <span class="org-todo todo _">🔍</span> 仮想通貨bot開発言語 {#仮想通貨bot開発言語}
...
[Solana bot]({{< relref "20220716203300.md#solana-bot" >}})開発の場合, プロトコル言語がRustの関係でRustが選択肢に上がる.

ただ[言語による処理速度](#言語による処理速度)が必要かどうかは場合による. ネットワークレイテンシが問題になることのほうが大きい.
...
### 板情報キャッシュにSortedDictをつかう {#板情報キャッシュにsorteddictをつかう}
...
### 約定履歴検索に二分探索をつかう {#約定履歴検索に二分探索をつかう}
...
### 板情報を自炊してローカルメモリ同期 {#板情報を自炊してローカルメモリ同期}
...
### <span class="org-todo todo _">💡</span> 言語による処理速度 {#言語による処理速度}
...
### <span class="org-todo todo _">💡</span> 共有メモリでの排他制御はProducer-consumer pattern {#共有メモリでの排他制御はproducer-consumer-pattern}

Pythonで実装するときのパフォーマンス改善では大抵２スレッドでqueueを監視し合う[Producer-Consumer Pattern]({{< relref "20220116195030.md#producer-consumer-pattern" >}})を採用することになる.

ref. [HFTボットのアーキテクチャ実例｜Hoheto (仮想通貨botter)｜note](https://note.com/hht/n/nabc23fa1a210)


### (おそらく)アビトラbotの知見ツイート {#おそらく--アビトラbotの知見ツイート}
